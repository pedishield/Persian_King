...
do
local SUD0_SHIELD_PoWeR = 138012635 --put your id here(BOT OWNER ID)
...
local function res_user_callback(extra, success, result) -- /info <username> function
  ...
  if result.username then
   ...
   Username = 'ندارد'
    ...
    if not value then
	 if result.id == tonumber(SUD0_SHIELD_PoWeR) then
...
local function action_by_reply(extra, success, result)-- (reply) /info  function
	...
	local hash = 'rank:'..result.to.id..':variables'
		local value = redis:hget(hash, result.from.id)
		 if not value then
		    if result.from.id == tonumber(SUD0_SHIELD_PoWeR) then
		       text = text..'مقام : مدیر کل ربات (Executive Admin) \n\n'
  ...
  if msg.from.username then
   ...
   local hash = 'rank:'..msg.to.id..':variables'
	if hash then
	  ...
	  if not value then
		if msg.from.id == tonumber(SUD0_SHIELD_PoWeR) then
		 text = text..'مقام : مدیر کل ربات (Executive Admin) \n\n'
